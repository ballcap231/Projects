...
-- ***** your code below *****
FROM events 
LEFT OUTER JOIN mortality ON events.patient_id = mortality.patient_id
...
-- ***** your code below *****
FROM events
INNER JOIN mortality ON events.patient_id = mortality.patient_id;
...
-- ***** your code below *****
FROM (SELECT count(event_id) AS event_count FROM alive_events GROUP BY patient_id) event_alive;
...
SELECT avg(event_count), min(event_count), max(event_count)
-- ***** your code below *****
FROM (SELECT count(event_id) AS event_count FROM dead_events GROUP BY patient_id) event_dead;
...
-- ***** your code below *****
FROM (SELECT COUNT(DISTINCT time) AS encounter_count FROM alive_events GROUP BY patient_id) encounter_alive;
...
SELECT avg(encounter_count), percentile(encounter_count, 0.5), min(encounter_count), max(encounter_count)
-- ***** your code below *****
FROM (SELECT COUNT(DISTINCT time) AS encounter_count FROM dead_events GROUP BY patient_id) encounter_dead;
...
-- ***** your code below *****
FROM (SELECT datediff(max(time), min(time)) as record_length FROM alive_events GROUP BY
patient_id) record_alive;
...
SELECT avg(record_length), percentile(record_length, 0.5), min(record_length), max(record_length)
-- ***** your code below *****
FROM (SELECT datediff(max(time), min(time)) as record_length FROM dead_events GROUP BY
patient_id) record_dead;
