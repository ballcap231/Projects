...
-- ***************************************************************************
eventswithmort = JOIN events BY patientid FULL OUTER, mortality by patientid;-- perform join of events and mortality by patientid;
...
Max_DT= GROUP aliveevents BY patientid;
Max_DT = FOREACH Max_DT GENERATE group AS patientid, 
                                             MAX(aliveevents.etimestamp) as index;
                                             
aliveevents = JOIN aliveevents BY patientid, Max_DT by patientid;
...
aliveevents = FOREACH aliveevents GENERATE aliveevents::patientid as patientid,
                                         ...
                                         DaysBetween(Max_DT::index, aliveevents::etimestamp) as time_difference;
