...
def convert_icd9(icd9_object):
    ...
    length_ = 3
    if icd9_str[0].isalpha() and not icd9_str.startswith('V'): length_ = 4
    if length_ >= len(icd9_str): return icd9_str
    return icd9_str[:length_]
...
def build_codemap(df_icd9, transform):
    ...
    zipped_digits = zip(df_digits, np.arange(len(df_digits)))
    codemap = dict(zipped_digits)
    return codemap
...
def create_dataset(path, codemap, transform):
    ...
    diag_group = df_diagnoses.groupby(['HADM_ID'])
    patient_group = df_admissions.groupby(['SUBJECT_ID'])
    data_in_seq = []
    patient_ids = []
    labels = []
    for patient_id, patient_info in patient_group:
        patient_ids.append(patient_id)
        label = df_mortality.loc[df_mortality.SUBJECT_ID == patient_id].MORTALITY.to_numpy()[0]
        labels.append(label)
        stored = []
        patient_info = patient_info.sort_values(by=['ADMITTIME'])
        for _, patient_visit in patient_info.iterrows():
            diags = diag_group.get_group(patient_visit['HADM_ID']).ICD9_CODE.to_numpy()
            diags = list(filter(lambda a: a in codemap.keys(), diags))
            stored.append(list(map(lambda a: codemap[a], diags)))
        data_in_seq.append(stored)
    return patient_ids, labels, data_in_seq
