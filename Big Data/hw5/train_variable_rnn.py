...
def predict_mortality(model, device, data_loader):
	...
	probas = []
	with torch.no_grad():
		for xx, (input_, target) in enumerate(data_loader):
			if isinstance(input_, tuple):
				input_ = tuple([xx.to(device) if type(xx) == torch.Tensor else xx for xx in input_])
			else:
				input_ = input_.to(device)
			pred_numpy = nn.Softmax()(model(input_)).numpy()
			pred_ls = pred_numpy[:, 1].tolist()
			probas.append(pred_ls[0])
	return probas
