...
def logistic_regression_pred(X_train, Y_train):
	...
	mymodel = LogisticRegression(random_state=RANDOM_STATE).fit(X_train,Y_train)
	return mymodel.predict(X_train)
...
def svm_pred(X_train, Y_train):
	...
	#use default params for the classifier
	mymodel = LinearSVC(random_state=RANDOM_STATE).fit(X_train,Y_train)
	return mymodel.predict(X_train)
...
def decisionTree_pred(X_train, Y_train):
	#train a logistic regression classifier using X_train and Y_train. Use this to predict labels of X_train
	#use max_depth as 5
	mymodel = DecisionTreeClassifier(random_state=RANDOM_STATE,max_depth=5).fit(X_train,Y_train)
	return mymodel.predict(X_train)
